#!/usr/bin/env bash
# Run this from inside ~/Jedida-market.com/frontend
# Self-contained: writes the offline-caching fix directly, no patch needed.
# Idempotent: safe to re-run.
set -e

echo "== Writing offline-cache fix files =="
mkdir -p "$(dirname "src/hooks/useCachedQuery.js")"
cat > "src/hooks/useCachedQuery.js" <<'FILE_EOF'
...
FILE_EOF
echo "  -> wrote src/hooks/useCachedQuery.js"

mkdir -p "$(dirname "src/pages/buyer/Marketplace.jsx")"
cat > "src/pages/buyer/Marketplace.jsx" <<'FILE_EOF'
...
FILE_EOF
echo "  -> wrote src/pages/buyer/Marketplace.jsx"

echo "== Done. Restart your frontend dev server (or rebuild) to pick this up. =="
